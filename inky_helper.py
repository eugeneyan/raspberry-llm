...
from inky_frame import button_a, button_b, button_c, button_d, button_e
...
def network_led_callback(t):
    ...
    brightness = (sin(ticks_ms() * pi * 2 / (1000 / network_led_pulse_speed_hz)) * 40) + 60
    value = int(pow(brightness / 100.0, 2.8) * 65535.0 + 0.5)
    network_led_pwm.duty_u16(value)
...
def sleep(t):
    ...
    interval = min(t, 255)  # Limit interval to the maximum allowed value

    for _ in range(t // interval):
        rtc.set_timer(interval, ttp=rtc.TIMER_TICK_1_OVER_60HZ)
        utime_sleep(interval)

    remaining_time = t % interval
    if remaining_time > 0:
        rtc.set_timer(remaining_time, ttp=rtc.TIMER_TICK_1_OVER_60HZ)
        utime_sleep(remaining_time)
...
def load_state():
    global state
    data = loads(open("/state.json", "r").read())
